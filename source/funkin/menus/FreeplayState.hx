...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		DiscordUtil.call("onMenuLoaded", ["F"]);

		super.create();

		// LOAD CHARACTERS

		bg = new FlxSprite(0, 0).loadAnimatedGraphic(Paths.image('menus/menuBGEffect'));
		bg.alpha = 0.7;
		bg.scale.set(1, 1);
		if (songs.length > 0)
		...
		coopText = new FlxText(diffText.x, diffText.y + diffText.height + 2, 0, "There is only you.", 24);
	 ...
	 */
	public var timeUntilAutoplay:Float = 10;
	/**
	 * Whenever the song autoplays when hovered over.
	 */
	public var disableAutoPlay:Bool = true;
	/**
	...
	public function select() {
		...
		FlxTween.tween(FlxG.camera, {zoom: 1.8, alpha: 0, angle: -3.7}, 1.5, {ease: FlxEase.quadInOut});
	...
	public var coopLabels:Array<String> = [
		"There is only you.",
		"you can play as me lol",
		"[TAB] you can play with your friends, if you had any",
		"[TAB] the same thing but switched"
	];
