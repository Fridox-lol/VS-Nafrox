...
class PlayState extends MusicBeatState
{
	 ...
	 */
	public var introSprites:Array<String> = ['game/we', 'game/ready', "game/set", "game/go"];
	/**
	...
	public var playingOnEasy:Bool = false; //detects if the song is being played on easy difficulty

	public var playingOnNormal:Bool = false; //same thing as playingOnEasy but for normal difficulty
	...
	@:dox(hide) function startSong():Void
	{
		...
		if (PlayState.difficulty == "easy" && isStoryMode) {
			playingOnEasy = true;
			trace('SONGNAME IS ' + PlayState.SONG.meta.displayName);
			trace('DIFFICULTY IS ' + PlayState.difficulty);
			trace('BAD ENDING ACTIVATED');
		}

		if (PlayState.difficulty == "normal" && isStoryMode) {
			playingOnNormal = true;
			trace('SONGNAME IS ' + PlayState.SONG.meta.displayName);
			trace('DIFFICULTY IS ' + PlayState.difficulty);
			trace('BAD ENDING ACTIVATED');
		}

		FlxG.save.data.lastSongPlayed = PlayState.SONG.meta.displayName;
	...
	public function nextSong() {
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				} //this better work i swear to god

				if (validScore)
				{
					FlxG.save.data.FIRSTTIMEBEATINGWEEK = true; //change to true after finished
					trace(FlxG.save.data.FIRSTTIMEBEATINGWEEK);
		...
		if (PlayState.SONG.meta.displayName == 'Null Object Audrey Remix'){
			FlxG.save.data.REMIXBEATEN = true;
		}

		if (playingOnEasy && PlayState.SONG.meta.displayName == 'Factory Reset') {
				trace('YOU HAVE UNLOCKED THE BAD ENDING');
				PlayState.loadSong("death", "normal");
				FlxG.switchState(new PlayState());
		}

		if (playingOnNormal && PlayState.SONG.meta.displayName == 'Factory Reset') {
			trace('YOU HAVE UNLOCKED THE BAD ENDING');
			PlayState.loadSong("death", "normal");
			FlxG.switchState(new PlayState());
	}
