...
function onGameOver() {
	DiscordUtil.changePresence('died.', PlayState.SONG.meta.displayName + " (" + PlayState.difficulty + ")");
}
...
function onPlayStateUpdate() {
	DiscordUtil.changeSongPresence(
		...
		(PlayState.instance.paused ? "Resting - " : "") + PlayState.SONG.meta.displayName + " (" + PlayState.difficulty + ")",
...
function onMenuLoaded(name:String) {
	...
	DiscordUtil.changePresenceSince("Failed to load DiscordUtil.changePresenceSince", null);
}
